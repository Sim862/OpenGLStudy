...
struct RenderEntry
{
	using RenderFunc = void(*)(Shader&, unsigned int, Model*);

	RenderFunc func;
	Shader* shader;   // 이 함수가 사용할 Shader
};
...
void renderSceneGeometry_Anim(Shader& shader, unsigned int cubeVAO, Model* model)
{
...
int main()
{
	...
	RenderEntry gRenderEntries[] = {
	{ &renderSceneGeometry,     &lightingShader }, // 정적 큐브용 셰이더
	{ &renderSceneGeometry_Anim, &anim }     // 애니메이션 모델용 셰이더
	};
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		renderSceneGeometry(depthShader, cubeVAO, &human);
